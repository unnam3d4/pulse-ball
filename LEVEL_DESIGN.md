...
## Replay loop: FLOW and Master goals

FLOW is the skill-expression layer. It does not change core physics or gate campaign progress.

FLOW increases from meaningful actions such as:
- collecting stars;
- activating Boost and Relay nodes;
- stomping enemies;
- destroying breakable blocks;
- completing switches/bridges;
- successful Orb interactions and Orb enemy hits.

The chain expires after a short inactivity window and resets on damage. Higher FLOW adds visual energy to the player and is recorded as the best FLOW for the run.

Every level also has one optional Master goal. Master completion never blocks the next level.

Chapter pattern:
1. first level: **Clean** — finish without taking damage;
2. second level: **FLOW** — reach the chapter FLOW target;
3. third level: **Perfect** — collect every star and finish without damage.

FLOW targets rise gradually by chapter: x3, x4, x5, x6, x7, x8.

Design implication: future routes should provide enough meaningful actions to sustain a skilled FLOW chain, but must not force players to maintain FLOW to finish the level.
